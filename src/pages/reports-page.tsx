...
export function ReportsPage() {
  ...
  const fetchData = async () => {
    ...
    const [payRes, expRes, bookRes, custRes] = await Promise.all([
      supabase
        .from("payments")
        .select(
          "*, booking:bookings(*, customer:customers(*), service:services(*))",
        ),
  ...
  const reportData = useMemo(() => {
    ...
    const grossRevenue = periodPayments.reduce(
    ...
    if (period === "daily") {
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        const rev = periodPayments
          .filter((p) => p.payment_date >= wsStr && p.payment_date <= weStr)
          .reduce((s, p) => s + Number(p.amount), 0);
        const exp = periodExpenses
          .filter((e) => e.expense_date >= wsStr && e.expense_date <= weStr)
          .reduce((s, e) => s + Number(e.amount), 0);
        chartData.push({
          label: `W${12 - i}`,
          revenue: rev,
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        const me = new Date(d.getFullYear(), d.getMonth() + 1, 0)
          ...
          .split("T")[0];
        const rev = periodPayments
          .filter((p) => p.payment_date >= ms && p.payment_date <= me)
          .reduce((s, p) => s + Number(p.amount), 0);
        const exp = periodExpenses
      ...
      for (let i = 4; i >= 0; i--) {
        ...
        const rev = periodPayments
          .filter((p) => p.payment_date >= ys && p.payment_date <= ye)
          .reduce((s, p) => s + Number(p.amount), 0);
        const exp = periodExpenses
          .filter((e) => e.expense_date >= ys && e.expense_date <= ye)
          .reduce((s, e) => s + Number(e.amount), 0);
        chartData.push({
          label: `${d.getFullYear()}`,
          revenue: rev,
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = `LaaLash-report-${period}-${new Date().toISOString().split("T")[0]}.csv`;
    a.click();
  ...
  const exportPDF = () => {
    ...
    if (!win) return;
    win.document.write(`
      <html><head><title>LaaLash Report</title>
      <style>
      ...
      <h1>LaaLash Studio - ${period.charAt(0).toUpperCase() + period.slice(1)} Report</h1>
    ...
    <PageTransition>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
        {[
          {
            label: "Gross Revenue",
            value: formatCurrency(reportData.grossRevenue),
            icon: TrendingUp,
          ...
          {
            label: "Appointments",
            value: String(reportData.numAppointments),
            icon: Calendar,
            color: "text-blue-500",
          },
          {
            label: "New Customers",
            value: String(reportData.numCustomers),
            icon: Users,
      ...
      <div className="rounded-2xl border border-border bg-card p-6 shadow-soft mb-6">
        <h3 className="text-lg font-serif font-semibold mb-4 capitalize">
          {period} Performance
        </h3>
