...
import { Sparkles, Calendar, Clock, Scissors, Star, ArrowRight, MapPin, Phone, Instagram, Heart } from 'lucide-react';
...
const DEFAULT_HERO = 'https://images.pexels.com/photos/5128222/pexels-photo-5128222.jpeg?auto=compress&cs=tinysrgb&h=650&w=940';
const DEFAULT_GALLERY_1 = 'https://images.pexels.com/photos/6135662/pexels-photo-6135662.jpeg?auto=compress&cs=tinysrgb&h=650&w=940';
const DEFAULT_GALLERY_2 = 'https://images.pexels.com/photos/5128234/pexels-photo-5128234.jpeg?auto=compress&cs=tinysrgb&h=650&w=940';
const DEFAULT_GALLERY_3 = 'https://images.pexels.com/photos/36930354/pexels-photo-36930354.jpeg?auto=compress&cs=tinysrgb&h=650&w=940';
...
export function LandingPage() {
  ...
  const businessName = settings?.business_name ?? 'LashFlow Studio';
  ...
  const features = [
    { icon: Calendar, title: cms?.feature_1_title ?? 'Easy Online Booking', desc: cms?.feature_1_desc ?? 'Pick your service and time in under a minute — no calls needed.' },
    { icon: Clock, title: cms?.feature_2_title ?? 'Flexible Scheduling', desc: cms?.feature_2_desc ?? 'Choose from available slots that fit your busy lifestyle.' },
    { icon: Heart, title: cms?.feature_3_title ?? 'Expert Care', desc: cms?.feature_3_desc ?? 'Professional lash treatments tailored to your unique style.' },
  ];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-rose-50 via-pink-50/30 to-amber-50/20">
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            ...
            >
              <div className="inline-flex items-center gap-2 rounded-full bg-primary/10 px-4 py-1.5 mb-6">
                ...
                <span className="text-xs font-medium text-primary">{cms?.hero_badge ?? 'Premium Lash Studio'}</span>
              </div>
              ...
              <p className="mt-5 text-base text-muted-foreground max-w-md leading-relaxed">
                {cms?.hero_subtitle ?? 'Book your lash appointment online in just a few taps. Choose your service, pick a time, and we\'ll handle the rest.'}
              </p>
              ...
              <div className="mt-8 flex items-center gap-6">
                <div className="flex items-center gap-1">
                  ...
                  <span className="ml-2 text-sm text-muted-foreground">Loved by clients</span>
                </div>
              </div>
            </motion.div>
            ...
            >
              <div className="relative rounded-3xl overflow-hidden shadow-soft-lg aspect-[4/5]">
                <img src={heroImage} alt="Lash extension treatment" className="w-full h-full object-cover" />
      ...
      {/* Services */}
      {services.length > 0 && (
        <section id="services" className="max-w-6xl mx-auto px-4 sm:px-6 py-16 md:py-20">
          <div className="text-center mb-10">
            <h2 className="text-3xl font-serif font-bold">Our Services</h2>
            <p className="mt-3 text-sm text-muted-foreground max-w-lg mx-auto">
              Explore our range of lash treatments designed to enhance your natural beauty.
            </p>
          </div>
      ...
      <section className="bg-white/60 border-y border-rose-100/50">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16">
          <div className="text-center mb-10">
            ...
            <p className="mt-3 text-sm text-muted-foreground">A glimpse into our lash studio.</p>
          </div>
      ...
      <section className="max-w-4xl mx-auto px-4 sm:px-6 py-16 md:py-24 text-center">
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <h2 className="text-3xl md:text-4xl font-serif font-bold">{cms?.cta_title ?? 'Ready to book?'}</h2>
          <p className="mt-4 text-base text-muted-foreground max-w-md mx-auto">
            {cms?.cta_subtitle ?? 'Schedule your lash appointment online today. It only takes a minute.'}
          </p>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
            ...
            <div className="flex items-center gap-5 text-sm text-muted-foreground">
              {settings?.phone && (
                <a href={`tel:${settings.phone}`} className="flex items-center gap-1.5 hover:text-foreground transition-colors">
                  <Phone className="h-4 w-4" />
                  {settings.phone}
                </a>
              )}
              {settings?.instagram && (
                <a href={`https://instagram.com/${settings.instagram}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 hover:text-foreground transition-colors">
